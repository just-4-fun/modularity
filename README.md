...
Modules that have no inbound dependencies and only coordinate or use other modules (top-level alike) can be subclasses of `BaseModule`. Whereas modules that are used by other modules and/or have non-trivial lifecycle (long initialization or restful) can extend the `Module` class. Modules that can have more than one implementation (like release and test) might consider extending `Module` class too.  
	...
	- consumes a substantial amount of resources or depends on some external resource that can be temporarily unavailable, so it can have “restful” lifecycle   
...
The entry point to the application is the container’s `moduleReference` method returning a `ModuleReference` instance which allows access the target module from outside of the container. It’s lightweight unless it binds the target module.   
...
4. **Finishing**: When the application receives a request to finish, the “main” module reference’s `unbind` method has to be called.  
...
Since then the application can exit the process or wait for the next request to start (back to Standby phase).  
...
- one of the container’s `ThreadContexts`:   
	- CONTAINER: a single-threaded instance shared among all users including the container itself. Can be overridden in container’s `onCreateThreadContext` callback  
...
It suits a situation when a container needs to talk to modules or a module needs to talk to its users. Communication via a feedback channel takes the following steps:  
...
- Implement the listener by the receiver module:
	...
	```  
- Create a channel in the sender (container or module) passing it the handler callback. Then use it at runtime to deliver feedback to listeners:
	``` kotlin
...
Except for the two natural factors that define module behavior (binding and requesting a module), there are three more:  
...
The `UIModule`’s counterpart is the `UiModuleReference` interface that can be implemented by an `Activity` to deal with an `UiModule` by declaring a target module access property. Like so:  
...
Also, the `FloatService` serves for switching app to the foreground that increases its process priority. The `startForeground` and `stopForeground` methods of a container control this behavior.  
The `FloatService` is registered in the library's AndroidManifest and merged into the host app's manifest:  
